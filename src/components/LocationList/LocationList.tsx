...
function LocationList({ locations }: LocationListProps) {
  ...
  const createList = (collection: LocationCollection) => {
    return collection.length === 0 ? (
      <li className={styles.location}>City not found</li>
    ) : (
      collection.map((location: Location) => {
        return (
          ...
          >
            {location.name}, {location.country}, {location.state}, Geo coords: [
  ...
  return <ul id={styles.locations}>{createList(locations)}</ul>;
}
